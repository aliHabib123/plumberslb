 ...
 */
    ...
    class projectPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new StringField('project_title');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('project_desc');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('project_img');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new IntegerField('project_order');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new IntegerField('show_home');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new IntegerField('upcoming');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        }
        ...
        protected function CreateGridSearchControl(Grid $grid)
        {
            ...
            $grid->SearchControl = new SimpleSearch('projectssearch', $this->dataset,
                array('project_id', 'project_title', 'project_desc', 'project_img', 'project_order', 'show_home', 'upcoming'),
                array($this->RenderText('Project Id'), $this->RenderText('Project Title'), $this->RenderText('Project Desc'), $this->RenderText('Project Img'), $this->RenderText('Project Order'), $this->RenderText('Show Home'), $this->RenderText('Upcoming')),
                array(
        ...
        protected function CreateGridAdvancedSearchControl(Grid $grid)
        {
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('project_title', $this->RenderText('Project Title')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('project_desc', $this->RenderText('Project Desc')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('project_img', $this->RenderText('Project Img')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('project_order', $this->RenderText('Project Order')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('upcoming', $this->RenderText('Upcoming')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for project_title field
            //
            $column = new TextViewColumn('project_title', 'Project Title', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('projectGrid_project_title_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            
            //
            // View column for project_desc field
            //
            $column = new TextViewColumn('project_desc', 'Project Desc', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('projectGrid_project_desc_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            
            //
            // View column for project_img field
            //
            $column = new ExternalImageColumn('project_img', 'Project Img', $this->dataset, '');
            ...
            //
            // View column for project_order field
            //
            $column = new TextViewColumn('project_order', 'Project Order', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for upcoming field
            //
            $column = new TextViewColumn('upcoming', 'Upcoming', $this->dataset);
            $column->SetOrderable(true);
            $column = new CheckBoxFormatValueViewColumnDecorator($column);
            $column->SetDisplayValues($this->RenderText('<img src="images/checked.png" alt="true">'), $this->RenderText(''));
            $column->SetDescription($this->RenderText(''));
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('projectGrid_project_title_handler_view');
            $grid->AddSingleRecordViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('projectGrid_project_desc_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for project_title field
            //
            $editor = new TextEdit('project_title_edit');
            $editor->SetSize(75);
            $editColumn = new CustomEditColumn('Project Title', 'project_title', $editor, $this->dataset);
            ...
            //
            // Edit column for project_desc field
            //
            $editor = new HtmlWysiwygEditor('project_desc_edit');
            $editor->SetAllowColorControls(true);
            $editColumn = new CustomEditColumn('Project Desc', 'project_desc', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for project_img field
            //
            $editor = new ImageUploader('project_img_edit');
            $editor->SetShowImage(true);
            $editColumn = new UploadFileToFolderColumn('Project Img', 'project_img', $editor, $this->dataset, false, false, '../public/uploads/images/');
            $editColumn->OnCustomFileName->AddListener('project_img_GenerateFileName_edit', $this);
            ...
            //
            // Edit column for project_order field
            //
            $editor = new TextEdit('project_order_edit');
            $editColumn = new CustomEditColumn('Project Order', 'project_order', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            ...
            //
            // Edit column for upcoming field
            //
            $editor = new ComboBox('upcoming_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $editor->AddValue('0', $this->RenderText('no'));
            $editor->AddValue('1', $this->RenderText('yes'));
            $editColumn = new CustomEditColumn('Upcoming', 'upcoming', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        protected function AddInsertColumns(Grid $grid)
        {
            ...
            $editor->SetShowImage(true);
            $editColumn = new UploadFileToFolderColumn('Project Img', 'project_img', $editor, $this->dataset, false, false, '../public/uploads/images/');
            $editColumn->OnCustomFileName->AddListener('project_img_GenerateFileName_insert', $this);
            $editColumn->SetReplaceUploadedFileIfExist(true);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
        ...
        protected function AddPrintColumns(Grid $grid)
        {
            ...
            // View column for project_img field
            //
            $column = new TextViewColumn('project_img', 'Project Img', $this->dataset);
            $column->SetOrderable(true);
            $grid->AddPrintColumn($column);
        ...
        public function project_img_GenerateFileName_edit(&$filepath, &$handled, $original_file_name, $original_file_extension, $file_size)
        {
        ...
        public function project_img_GenerateFileName_insert(&$filepath, &$handled, $original_file_name, $original_file_extension, $file_size)
        {
        $targetFolder = FormatDatasetFieldsTemplate($this->GetDataset(), '../public/uploads/images/');
        FileUtils::ForceDirectories($targetFolder);
        ...
        protected function CreateGrid()
        {
            ...
            $column = new TextViewColumn('project_title', 'Project Title', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'projectGrid_project_title_handler_list', $column);
            ...
            $column = new TextViewColumn('project_desc', 'Project Desc', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'projectGrid_project_desc_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for project_title field
            //
            $column = new TextViewColumn('project_title', 'Project Title', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'projectGrid_project_title_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for project_desc field
            //
            $column = new TextViewColumn('project_desc', 'Project Desc', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'projectGrid_project_desc_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
